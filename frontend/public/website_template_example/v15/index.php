...
<head>
  ...
  <title>Media-katalog (v15 - live data)</title>
  <style>
    :root{
      --bg:#0b1020;
      --panel:#121a33;
...
<header>
  ...
  <div style="color:var(--muted); font-size:12px;">v15 · live fra databasen</div>
</header>
  ...
  <aside>
    <div class="filter">
      ...
      <div style="color:var(--muted); font-size:12px; margin-bottom:10px;">
        (Faktisk spørring mot <code>content</code>-tabellen, se api.php)
      </div>
...
<script>
  // v15: data hentes nå live fra databasen via api.php (content +
  // content_in_physical_collection + physical_collection + content_external_source),
  // i stedet for hardkodet demo-data som i v14. Selve rendering-logikken
  // under er uendret fra v14.
  let data = [];
  ...
  // Hent ekte data fra databasen via api.php, og render deretter.
  async function loadData(){
    renderChips();
    grid.innerHTML = `<div style="color:var(--muted); font-size:13px;">Laster data fra databasen…</div>`;
    try {
      ...
      grid.innerHTML = `<div style="color:#ff8080; font-size:13px;">Klarte ikke å hente data: ${err.message}</div>`;
      return;
    }
    render();
  }
