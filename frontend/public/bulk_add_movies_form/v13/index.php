...
<body class="bg-light">
  <div class="container py-4">
    ...
    <div class="card mb-3">
      <div class="card-body">
        <div class="row g-3">
          <div class="col-md-8">
            <label class="form-label" for="storageId">Lagringsplass-ID</label>
            <input
              type="text"
              class="form-control mono"
              id="storageId"
              value="564a3999-5d00-11f1-9526-bab3c527eb51"
            />
            <div class="form-text">ID for hvilken lagringsplass som skal brukes.</div>
          </div>
        </div>
      </div>
    </div>
              ...
              <table class="table table-sm align-middle" id="discEditorTable">
                <thead class="table-light">
                  <tr>
                    ...
                    <th style="width: 180px;">Legg til i lagringsplass</th>
  ...
  <script>
    ...
    const DEFAULT_STORAGE_ID = '564a3999-5d00-11f1-9526-bab3c527eb51';
    ...
    function addDiscEditorRow({
      ...
      const row = el(`
        <tr data-bonus-items="[]">
          ...
          <td><input class="form-control form-control-sm" name="storage_slot_no" type="number" min="1" placeholder="NULL" value="${escapeHtml(safeStorageSlotNo)}"></td>
          <td class="text-center"><input class="form-check-input" name="add_to_storage" type="checkbox" ${add_to_storage ? 'checked' : ''}></td>
          <td class="nowrap">
    ...
    document.getElementById('btnSaveDiscsForSingle').addEventListener('click', () => {
      ...
      const discs = [...discEditorTbody.querySelectorAll('tr')].map(tr => {
        ...
        const storage_slot_no_raw = tr.querySelector('input[name="storage_slot_no"]').value.trim();
        const storage_slot_no = storage_slot_no_raw === "" ? null : Number(storage_slot_no_raw);
    ...
    function addBoxDiscRow(boxSetRoot, {
      ...
      const row = el(`
        <tr data-bonus-items="[]" data-related-index="${escapeHtml(relatedIndex)}" data-storage-slot-no="${escapeHtml(safeStorageSlotNo)}" data-add-to-storage="${add_to_storage ? '1' : '0'}">
          ...
          <td data-role="disc-add-to-storage">${add_to_storage ? 'Ja' : 'Nei'}</td>
    ...
    function createBoxSetCard() {
                ...
                <div class="card-body">
                  <div class="row g-2 align-items-end mb-2">
                    ...
                    <div class="col-md-2">
                      ...
                      <input class="form-control" name="disc_storage_slot_no" type="number" min="1" placeholder="NULL">
                    </div>
                    <div class="col-md-2">
                      <div class="form-check mt-4">
                        <input class="form-check-input" type="checkbox" name="disc_add_to_storage" id="${boxSetId}_disc_add_to_storage">
                        <label class="form-check-label" for="${boxSetId}_disc_add_to_storage">Legg til i lagringsplass</label>
                      </div>
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="col-md-12">
                      <button class="btn btn-primary" type="button" data-action="add-disc">${escapeHtml(fmt('btn.add_disc'))}</button>
                    </div>
                  </div>
                    ...
                    <table class="table table-sm align-middle mb-0">
                      <thead class="table-light">
                        <tr>
                          ...
                          <th style="width: 150px;">I lagringsplass</th>
      ...
      root.querySelector('[data-action="add-disc"]').addEventListener('click', () => {
        ...
        const storage_slot_no_raw = root.querySelector('input[name="disc_storage_slot_no"]').value.trim();
        const storage_slot_no = storage_slot_no_raw === "" ? null : Number(storage_slot_no_raw);
    ...
    function readAllBoxSets() {
      ...
      return cards.map((root, idx) => {
        ...
        const discs = [...root.querySelectorAll('tbody[data-role="discs"] tr')].map(tr => {
          ...
          const bonus_items = getJsonAttr(tr, 'data-bonus-items', []);
          const storage_slot_no_raw = tr.getAttribute('data-storage-slot-no');
          const storage_slot_no = storage_slot_no_raw === "" || storage_slot_no_raw == null ? null : Number(storage_slot_no_raw);
    ...
    const STORAGE_KEY = 'bulkAddState_v13';
    let saveTimer = null;
      ...
      (state.boxSets || []).forEach(bs => {
        ...
        (bs.discs || []).forEach(d => {
          addBoxDiscRow(root, {
            ...
            storage_slot_no: d.storage_slot_no ?? '',
