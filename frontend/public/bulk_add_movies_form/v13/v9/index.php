...
/**
 * i18n:
 * - Switch language via ?lang=en or ?lang=nb
 * - Stored in session
 * - Texts live in lang/<lang>.php (easy to edit)
 */
...
function tr(array $t, string $key): string {
  return $t[$key] ?? $key; // fallback to key so missing strings are obvious
}
?>
<!doctype html>
<html lang="<?= h($lang) ?>">
...
<body class="bg-light">
  <div class="container py-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
      ...
      <div class="d-flex align-items-center gap-2">
        <!-- Language switcher (reloads page; state is preserved via localStorage) -->
    ...
    <div class="tab-content border border-top-0 bg-white p-3 rounded-bottom">
      ...
      <div class="tab-pane fade show active" id="single" role="tabpanel" aria-labelledby="single-tab">
        ...
        <div class="row g-3 mb-3">
          <div class="col-md-4">
            <label class="form-label" for="singleFormat"><?= h(tr($t, 'label.format')) ?></label>
            <select class="form-select" id="singleFormat">
              <option value="DVD"><?= h(tr($t, 'format.dvd')) ?></option>
              <option value="BD" selected><?= h(tr($t, 'format.bluray')) ?></option>
              <option value="UHD"><?= h(tr($t, 'format.uhd')) ?></option>
            </select>
          </div>
          <div class="col-md-4">
          ...
          <table class="table table-sm align-middle" id="singleTable">
            <thead class="table-light">
              <tr>
                ...
                <th style="min-width: 180px;"><?= h(tr($t, 'col.imdb')) ?></th>
  ...
  <script>
    // Give JS access to i18n strings (for dynamic labels)
    ...
    // =========================================================================
    // Modal freeze logic: when bonus modal is open, freeze disc modal if present
    // =========================================================================
    ...
    // =========================================================================
    // Bonus modal (shared for single-disc rows + box-set disc rows)
    // =========================================================================
    ...
    function addDiscEditorRow({type_disc="feature", format="BD", label="", bonus_items=[]} = {}) {
      const row = el(`
        <tr data-bonus-items="[]">
          ...
          <td><input class="form-control form-control-sm" name="label" placeholder="${escapeHtml(fmt('ph.optional_label'))}" value="${escapeHtml(label)}"></td>
          <td class="nowrap">
      ...
      const updateBonusLabel = () => {
        ...
        btn.textContent = (items && items.length) ? fmt('btn.edit_count', {n: items.length}) : fmt('btn.edit');
      };
      updateBonusLabel();
    ...
    function addSingleRow({title = "", barcode = "", imdb = ""} = {}) {
      const row = el(`
        <tr data-discs="[]">
          ...
          <td><input class="form-control form-control-sm" name="imdb" placeholder="${escapeHtml(fmt('ph.imdb'))}" value="${escapeHtml(imdb)}"></td>
          <td class="nowrap">
    ...
    document.getElementById('btnSingleAddRow').addEventListener('click', () => { addSingleRow(); scheduleSave(); });
    ...
    function addBoxTitleRow(boxSetRoot, {title="", imdb="", innerEan="", treatAsSingle=null} = {}) {
      ...
      const row = el(`
        <tr>
          ...
          <td><input class="form-control form-control-sm" name="title" placeholder="${escapeHtml(fmt('ph.title'))}" value="${escapeHtml(title)}"></td>
          <td><input class="form-control form-control-sm" name="imdb" placeholder="${escapeHtml(fmt('ph.imdb'))}" value="${escapeHtml(imdb)}"></td>
          <td><input class="form-control form-control-sm" name="inner_ean" placeholder="${escapeHtml(fmt('ph.ean13'))}" value="${escapeHtml(innerEan)}"></td>
          <td>
          ...
          <td><button class="btn btn-sm btn-outline-danger" type="button" aria-label="${escapeHtml(fmt('aria.remove_movie'))}">✕</button></td>
        </tr>
      `);
    ...
    function addBoxDiscRow(boxSetRoot, {typeDisc="feature", format="BD", relatedTitle="", bonus_items=[]} = {}) {
      ...
      row.querySelector('button[data-action="edit-bonus"]').addEventListener('click', () => {
        ...
        const subtitle = fmt('subtitle.box_disc', {n: order, discType, discFormat, rel: rel || fmt('word.whole_box_plain')});
    ...
    function createBoxSetCard() {
              ...
              <div class="row g-3 mb-3">
                <div class="col-md-4">
                  ...
                  <select class="form-select" name="box_format">
                    <option value="DVD">${escapeHtml(fmt('format.dvd'))}</option>
                    <option value="BD" selected>${escapeHtml(fmt('format.bluray'))}</option>
                    <option value="UHD">${escapeHtml(fmt('format.uhd'))}</option>
                  </select>
                </div>
                <div class="col-md-4">
                ...
                <table class="table table-sm align-middle">
                  <thead class="table-light">
                    <tr>
                      ...
                      <th style="min-width: 180px;">${escapeHtml(fmt('col.imdb'))}</th>
      ...
      root.querySelector('[data-action="add-disc"]').addEventListener('click', () => {
        ...
        addBoxDiscRow(root, {typeDisc, format, relatedTitle});
        updateSummary();
        scheduleSave();
      });
    ...
    // =========================================================================
    // localStorage persistence (so language switch does NOT wipe the form)
    // =========================================================================
    const STORAGE_KEY = 'bulkAddState_v1';
    let saveTimer = null;
    ...
    function restoreState(state) {
      ...
      // Singles
      if (state.singles?.format) document.getElementById('singleFormat').value = state.singles.format;
      ...
      (state.singles?.rows || []).forEach(r => {
        addSingleRow({ title: r.title, barcode: r.barcode, imdb: r.imdb });
        const tr = singleTbody.lastElementChild;
        if (tr) {
          setJsonAttr(tr, 'data-discs', Array.isArray(r.discs) ? r.discs : []);
          updateSingleDiscButtonLabel(tr);
        }
      });
      ...
      (state.boxSets || []).forEach(bs => {
        ...
        (bs.discs || []).forEach(d => {
          addBoxDiscRow(root, d);
          const tr = discsTbody.lastElementChild;
          if (tr) {
            setJsonAttr(tr, 'data-bonus-items', Array.isArray(d.bonus_items) ? d.bonus_items : []);
            const btn = tr.querySelector('button[data-action="edit-bonus"]');
            if (btn) {
              const n = (d.bonus_items || []).length;
              btn.textContent = n ? fmt('btn.edit_count', {n}) : fmt('btn.edit');
            }
          }
        });
    ...
    // =========================================================================
    // Init (then restore if we have state)
    // =========================================================================
    addSingleRow();
    accordion.appendChild(createBoxSetCard());
