...
<head>
  ...
  <title>Bulk add – Singles & Box sets (multi) + Bonus items</title>
...
<body class="bg-light">
  <div class="container py-4">
    ...
    <div class="collapse mb-3" id="help">
      <div class="card card-body">
        <ul class="mb-0">
          <li><strong>Bonus items</strong> are attached to a disc row (intended for <span class="mono">type_disc=bonus</span>).</li>
          <li>They correspond to table <span class="mono">disc_bonus_item</span>: <span class="mono">(disc_id, seq_no, title, item_type, runtime_seconds, notes)</span>.</li>
          <li>This is a frontend-only prototype: items are stored as JSON on the disc row via <span class="mono">data-bonus-items</span>.</li>
        </ul>
      </div>
    </div>
    ...
    <div class="tab-content border border-top-0 bg-white p-3 rounded-bottom">
      ...
      <!-- =========================
           SINGLE RELEASES (no bonus editor here yet)
      ========================== -->
      <div class="tab-pane fade show active" id="single" role="tabpanel" aria-labelledby="single-tab">
        ...
        <div class="alert alert-info">
          Disc editor is still a placeholder for singles in this prototype (bonus editor is implemented for box-set discs).
        </div>
      </div>

      <!-- =========================
           BOX SETS (MULTI) + BONUS ITEMS EDITOR
      ========================== -->
      <div class="tab-pane fade" id="box" role="tabpanel" aria-labelledby="box-tab">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
          <div>
            ...
            <div class="text-muted small">Add multiple box sets, then preview all at once. Bonus items can be edited per disc.</div>
          </div>
          <div class="d-flex gap-2">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="alert alert-warning py-2 d-none" id="bonusNotBonusAlert">
              This disc is not marked as <span class="mono">bonus</span>. You can still add items, but it might be inconsistent with your rules.
            </div>
  ...
  <script>
    ...
    // =========================================================================
    // Box sets (multi) + Bonus editor wiring
    // =========================================================================
    ...
    let bonusTargetDiscRow = null; // <tr> for the disc row being edited
    ...
    function updateBonusButtonLabel(discRow) {
      const items = getJsonAttr(discRow, 'data-bonus-items', []);
      const btn = discRow.querySelector('button[data-action="edit-bonus"]');
      if (!btn) return;
    ...
    function addDiscRow(boxSetRoot, {typeDisc="feature", format="BD", relatedTitle=""} = {}) {
      ...
      const row = el(`
        <tr data-bonus-items="[]">
          ...
          <td data-role="disc-related">${relatedTitle ? escapeHtml(relatedTitle) : ''}</td>
          <td>
      ...
      row.querySelector('button[data-action="edit-bonus"]').addEventListener('click', () => {
        openBonusEditorForDiscRow(row);
      });

      tbody.appendChild(row);
      updateBonusButtonLabel(row);
    }

    // -------------------------------------------------------------------------
    // Bonus editor modal logic
    // -------------------------------------------------------------------------
    ...
    function addBonusItemEditorRow({seq_no="", title="", item_type="featurette", runtime_seconds="", notes=""} = {}) {
      const row = el(`
        <tr>
          ...
          <td><button class="btn btn-sm btn-outline-danger" type="button" aria-label="Remove bonus item">✕</button></td>
        </tr>
      `);
    ...
    function openBonusEditorForDiscRow(discRow) {
      bonusTargetDiscRow = discRow;
      clearBonusTable();

      const discType = discRow.querySelector('[data-role="disc-type"]')?.textContent?.trim() || '';
      const discFormat = discRow.querySelector('[data-role="disc-format"]')?.textContent?.trim() || '';
      const discRelated = discRow.querySelector('[data-role="disc-related"]')?.textContent?.trim() || '';
      const order = discRow.children[0].textContent.trim();

      bonusModalSubtitle.textContent =
        `Disc #${order} · type=${discType} · format=${discFormat}` +
        (discRelated ? ` · related="${discRelated}"` : ' · related=(whole box)');

      bonusNotBonusAlert.classList.toggle('d-none', discType === 'bonus');

      const existing = getJsonAttr(discRow, 'data-bonus-items', []);
      (existing || []).forEach(item => addBonusItemEditorRow(item));

      // If empty, start with one row for convenience
    ...
    document.getElementById('btnSaveBonusItems').addEventListener('click', () => {
      ...
      // Basic cleanup: remove empty-title rows
      const cleaned = items.filter(x => (x.title && x.title.trim()) || x.seq_no);

      // Optional: sort by seq_no when possible
      cleaned.sort((a, b) => (a.seq_no ?? 0) - (b.seq_no ?? 0));

      setJsonAttr(bonusTargetDiscRow, 'data-bonus-items', cleaned);
      updateBonusButtonLabel(bonusTargetDiscRow);

      bonusTargetDiscRow = null;
      clearBonusTable();
    });
