...
<body class="bg-light">
  ...
  <script>
    ...
    // =========================================================================
    // Freeze logic for nested modals
    // =========================================================================
    ...
    // Singles row includes hidden tmdb_id
    function addSingleRow({title = "", format = null, barcode = "", imdb = "", tmdb = ""} = {}) {
      ...
      // IMPORTANT: tell TMDB modal which row to fill (both imdb + hidden tmdb)
    ...
    // Box movie row includes hidden tmdb_id + search button
    ...
    document.getElementById('btnPreviewAllBoxSets').addEventListener('click', () => {
      showPreview({ kind: "box_sets_bulk" });
    });
    ...
    // =========================================================================
    const STORAGE_KEY = 'bulkAddState_v3'; // bump schema
    let saveTimer = null;
  ...
  <!-- Must be the BULK version of script.js that writes to window.__bulkActiveImdbInput and window.__bulkActiveTmdbInput -->
